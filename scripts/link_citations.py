...
def trellis(*args):
    r = subprocess.run(
        ["trellis"] + list(args), capture_output=True, text=True, timeout=15
    )
    return r
...
for i, (doi, slug) in enumerate(doi_to_slug.items()):
    ...
    for ref in refs:
        ...
        r = trellis(
            "link", slug, target_slug, "--relation", "cites", "--actor-id", "daedalus"
        )
        if r.returncode == 0:
            linked += 1
    ...
    if (i + 1) % 50 == 0:
        ...
        eta = (total - i - 1) / rate
        print(
            f"  [{i+1}/{total}] links={linked} errors={errors} | {rate:.1f}/s ETA:{eta/60:.0f}m"
        )
