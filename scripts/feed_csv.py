...
"""
import csv, sys, os, time, json, subprocess, unicodedata

sys.path.insert(0, os.path.dirname(__file__))
from ingest import ingest_paper, _norm_title
...
doi_cache = {}
title_cache = {}
r = subprocess.run(['trellis','find','--tag','pipeline:scaffolded','--json','--limit','5000'],
...
for i, row in enumerate(rows):
    ...
    if (i + 1) % 100 == 0:
        ...
        rate = (i+1)/elapsed
        eta = (total - i - 1)/rate
        print(f"  [{i+1}/{total}] +{added} skip={skipped} fail={failed} | {rate:.0f}/s ETA:{eta/60:.0f}m")
